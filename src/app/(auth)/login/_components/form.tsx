...
const LoginForm = () => {
  ...
  const methods = useForm<z.infer<typeof loginSchema>>({ resolver: zodResolver(loginSchema) });
  const {
    watch,
    control,
    handleSubmit,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Form {...methods}>
        ...
        <Button
          disabled={!Boolean(watch('email') && watch('password')) || isLoading}
          type="submit"
        ...
        >
          {isLoading ? <ArrowPathIcon className="h-6 w-6 animate-spin" /> : 'Login'}
        </Button>
      </Form>
    </form>
  );
